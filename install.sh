      ...
      @   @   @@@@@@   @   @        @@@@@@@@@@  @@@@@@@@@@  @@@     @@   @@@@@@@@@@ @@@     @@@  @@  @@@
       @       @@@@       @                @@@                                  @@@
        @@      @@      @@          @@@@@@@@@                            @@@@@@@@@
         ...
         Offensive Security Manager — Community Edition
...
INSTALL_DIR="/opt/gungnir"
DB_NAME="allsafe_gungnir"
DB_USER="gungnir"
BACKEND_PORT=3006
...
cd "$INSTALL_DIR/frontend" && npm install --silent && npm run build --silent
success "Frontend built"

info "Configuring nginx..."
cat > /etc/nginx/sites-available/gungnir <<NGINX
server {
    listen 80;
    server_name ${DOMAIN};
    ...
    location /uploads/ { alias ${INSTALL_DIR}/backend/uploads/; }
}
NGINX
...
pm2 start server.js --name gungnir
pm2 save
pm2 startup | tail -1 | bash &>/dev/null || true
success "PM2 process 'gungnir' started"

echo -e "\n${GREEN}╔══════════════════════════════════════════╗"
echo -e "║   Gungnir Community installed!           ║"
echo -e "╚══════════════════════════════════════════╝${NC}"
echo -e "  URL:       http://${DOMAIN}"
echo -e "  User:      admin"
echo -e "  Password:  admin123  ${RED}(change immediately!)${NC}"
echo -e "  PM2:       pm2 logs gungnir"
